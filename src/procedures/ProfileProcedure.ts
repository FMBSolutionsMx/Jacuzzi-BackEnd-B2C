...
export default async function ProfileProcedure(
  ...
  // return results.recordset[0];
    ...
    try {
      let resHana= await sh.statements(`CALL _E_HANDEL_B2C.PROFILES('${model.action}','${model.business}','${model.id || 0}','${model.cardCode}','${model.favorites}','${model.shoppingCart}','${model.backOrder}','${model.localStorageFront}')`);
      if (model.action === "getAddresses") {
        return resHana;
      }
      return resHana[0] || [];
