...
export default async function Collection (model: CollectionModel) : Promise<any> {
  ...
  if(GlobalBusiness[0].type === 'SQL'){
    ...
    try {
      let resHana= await sh.statements(`CALL _E_HANDEL_B2C.COLLECTION('${model.action}','${model.business}','${model.docEntry}','${model.table}','${model.cardCode}','${model.initialDate}','${model.finalDate}')`);
      return resHana || [];
    } catch (error) {
