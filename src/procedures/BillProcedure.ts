...
export default async function BillingProcedure (model: BillModel) : Promise<any> {
  ...
  if(GlobalBusiness[0].type === 'SQL'){
    ...
    try {
      let resHana= await sh.statements(`CALL _E_HANDEL_B2C.BILL('${model.action}','${model.business}','${model.docEntry}','${model.table}','${model.cardCode}','${model.initialDate}','${model.finalDate}')`);
      //console.log("taxa",resHana);
      return resHana || [];
    } catch (error) {
      logger.error("BILL: ",error);
      return [];
    }
  }
}