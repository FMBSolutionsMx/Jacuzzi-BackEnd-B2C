...
export default async function Quotation (model: QuotationModel) : Promise<any> {
  ...
  if(GlobalBusiness[0].type === 'SQL'){
    ...
    try {
      let resHana= await sh.statements(`CALL _E_HANDEL_B2C.QUOTATION('${model.action}','${model.business}','${model.docEntry}','${model.table}','${model.cardCode}','${model.initialDate}','${model.finalDate}')`);
      return resHana || [];
    } catch (error) {
      logger.error("QUOTATION: ",error);
      return [];
    }
  }
}